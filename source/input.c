...
class_test((pba->Omega_ini_CMaDE_de <= 0.0) ||
  ...
  if((pba->Omega0_CMaDE_de <= 0.0)||(pba->Omega0_CMaDE_dm <= 0.0)||(pba->Omega_ini_CMaDE_de <= 0.0)||(pba->Omega_ini_CMaDE_dm <= 0.0 )){
    ...
    pba->Omega_ini_CMaDE_de =  0.18730875095302754;
    pba->Omega0_CMaDE_dm =  0.261206;
    pba->Omega0_CMaDE_de = 0.690026;
    pba->Omega0_k = 0.0;
    pba->k_c=1;
    pba->Q_coupling=1.0;    


      printf("No se han definido correctamente los parámetros de CMaDE. Se han asignado valores por defecto. \n");
      printf("Omega_ini_CMaDE_dm = %e \n", pba->Omega_ini_CMaDE_dm);
      printf("Omega_ini_CMaDE_de = %e \n", pba->Omega_ini_CMaDE_de);
      printf("Omega0_CMaDE_dm = %e \n", pba->Omega0_CMaDE_dm);
      printf("Omega0_CMaDE_de = %e \n", pba->Omega0_CMaDE_de);
      printf("Omega0_b = %e \n", pba->Omega0_b);
      printf("Omega0_g = %e \n", pba->Omega0_g);
      printf("Omega0_ur = %e \n", pba->Omega0_ur);
      printf("Omega0_k = %e \n", pba->Omega0_k);
      printf("k_c = %e \n", pba->k_c);
      printf("Q_coupling = %e \n", pba->Q_coupling);
      printf("Estos valores estan basados en los datos de Planck 2018 y corresponden a h=0.678100. \n");
      printf("Para el correcto funcionamiento, se deben ingresar obligatoriamente: \n");
      printf("Omega_ini_CMaDE_dm \n");
      printf("Omega_ini_CMaDE_de \n");
      printf("Omega0_CMaDE_dm \n");
      printf("Omega0_CMaDE_de \n");
  }
    */
  ...
  /* Read */
  //CLaudio Salas Pérez. 30/10/2025
  ...
  if ((ppt->gauge == synchronous) && (pba->Omega0_cdm < ppr->Omega0_cdm_min_synchronous) && (pba->has_CMaDE == _FALSE_)) {
 ...
 // Claudio Salas Pérez: 29/09/2025
   // Ajuste para el presupuesto de energía con CMaDE
   // Si CMaDE está activado, desactivar CDM, Lambda y Curvatura
  if (pba->has_CMaDE == _TRUE_) {
// Ajuste automático: desactivar CDM, Lambda y Curvatura
 
    class_test((pba->Omega0_cdm != 0.0),
  errmsg,
  "No se debe definir materia oscura fria si estamos usando CMaDE, en su luagr define Omega_CMaDE_dm.");
    Omega_tot = pba->Omega0_CMaDE_de + pba->Omega0_CMaDE_dm;
  }
  ...
  /* Step 2 */
  if ((flag1 == _FALSE_) && (pba->has_CMaDE == _FALSE_)) {
    /* Fill with Lambda */
  ...
  else if ((flag2 == _FALSE_) && (pba->has_CMaDE == _FALSE_)) {
    /* Fill up with fluid */
  ...
  else if ((flag3 == _TRUE_) && (param3 < 0.) && (pba->has_CMaDE == _FALSE_)) {
    ...
    if (input_verbose > 0){
      printf(" -> matched budget equations by adjusting Omega_scf = %g\n",pba->Omega0_scf);
                                   ...
                                   // Claudio Salas Pérez: 29/09/2025 
                                  // Para que se llene automáticamente el presupuesto de energía, CMaDE debe estar desactivado
