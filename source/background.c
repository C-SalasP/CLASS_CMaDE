...
int background_functions(
  ...
  rho_m += pvecback[pba->index_bg_rho_b];
    ...
    // Contribución de CMaDE al fondo
    if(pba->has_CMaDE == _TRUE_){
      pvecback[pba->index_bg_CMaDE_rho_dm] = pvecback_B[pba->index_bi_CMaDE_rho_dm];
      pvecback[pba->index_bg_CMaDE_rho_de] = pvecback_B[pba->index_bi_CMaDE_rho_de]; 
      

      p_tot += 0.0; 
      p_tot -= pvecback[pba->index_bg_CMaDE_rho_de];  
      rho_tot += pvecback[pba->index_bg_CMaDE_rho_dm];
      rho_m += pvecback[pba->index_bg_CMaDE_rho_dm];
      rho_tot += pvecback[pba->index_bg_CMaDE_rho_de];
        }
  ...
  if (pba->has_idm == _TRUE_) {
    ...
    rho_m += pvecback[pba->index_bg_rho_idm];
      ...
      static int idm_count = 0;
    if (idm_count == 0) {
      printf("Contribución de idm.\n");
      idm_count = 1;
      }
  }
  ...
  pvecback[pba->index_bg_H] = sqrt(rho_tot-pba->K/a/a);
    ...
    //En caso de ser necesario, aqui se pueden agregar los parametros w para ecuaciones de estado efectivas
    //Y asi agregegar densidades y presiones efectivas
  if(pba->has_CMaDE == _TRUE_){
      //esto es derivada con respecto a tiempo conforme
      //d/dtau = a^2 H d/da = a^2 H 1/a d/dloga
      pvecback[pba->index_bg_CMaDE_M_prime]  = (pba->Q_coupling)*(sqrt(6.0)*pow(pvecback[pba->index_bg_CMaDE_rho_de],3.0/2.0)/(_PI_));   
    }
...
int background_indices(
  ...
  // Claudio Salas Pérez   28/09/2025
  //inicializo todos los indices para CMaDE
  class_define_index(pba->index_bg_CMaDE_rho_dm,pba->has_CMaDE,index_bg,1);
  class_define_index(pba->index_bg_CMaDE_rho_de,pba->has_CMaDE,index_bg,1);

  //Claudio Salas Pérez   05/10/2025
  class_define_index(pba->index_bg_CMaDE_P_dm,pba->has_CMaDE,index_bg,1);
  class_define_index(pba->index_bg_CMaDE_W_eff_dm,pba->has_CMaDE,index_bg,1);
  class_define_index(pba->index_bg_CMaDE_P_de,pba->has_CMaDE,index_bg,1);
  class_define_index(pba->index_bg_CMaDE_W_eff_de,pba->has_CMaDE,index_bg,1);
  class_define_index(pba->index_bg_CMaDE_M_prime,pba->has_CMaDE,index_bg,1);
  ...
  // Claudio Salas Pérez   28/09/2025
  //inicializo todos los indices diferenciales para CMaDE 

  class_define_index(pba->index_bi_CMaDE_rho_dm,pba->has_CMaDE,index_bi,1);
  class_define_index(pba->index_bi_CMaDE_rho_de,pba->has_CMaDE,index_bi,1);
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"a",_TRUE_);
...
int background_derivs(
 ...
 //Claudio Salas Pérez: 29/09/2025
 // Sistemas de ecuaciones diferenciales para CMaDE
 if(pba->has_CMaDE == _TRUE_){    
    dy[pba->index_bi_CMaDE_rho_de] = pba->Q_coupling *sqrt(6.0)*pow(y[pba->index_bi_CMaDE_rho_de],3.0/2.0)/(a* H *_PI_);
    dy[pba->index_bi_CMaDE_rho_dm] = -3.0 * y[pba->index_bi_CMaDE_rho_dm] -pba->k_c*dy[pba->index_bi_CMaDE_rho_de];
}
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if (pba->has_idr == _TRUE_) {
      ...
      //Claudio Salas Pérez: 29/09/2025
      //Aqui hay una pequeña modificacion para que imprima CMaDE
    if ((pba->has_lambda == _TRUE_) || (pba->has_fld == _TRUE_) || 
    (pba->has_scf == _TRUE_) || (pba->has_curvature == _TRUE_) || 
