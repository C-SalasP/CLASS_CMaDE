 ...
 * time. Then, to perform a few manipulations of these terms in order toa
...
int perturbations_init(
  ...
  class_test((ppt->gauge == synchronous) && (pba->has_cdm == _FALSE_) && (pba->has_CMaDE == _FALSE_),
             ppt->error_message,
      ...
      for (index_k = ppt->k_size[index_md]-1; index_k >=0; index_k--) {
        ...
        class_run_parallel(with_arguments(ppr,pba,pth,ppt,index_md,index_ic,index_k),
          ...
          class_call(perturbations_workspace_init(ppr,
              ...
              //regresar
            // Claudio Salas Pérez: 17/10/2025
            if(pba->save_perturbations == _TRUE_){
            if (index_k % 1 == 0 || index_k == ppt->k_size[index_md]-1) {
            int block_id = (index_k);
            char filename[256];
            
            if (pba->has_CMaDE == _TRUE_) {
              sprintf(filename, "LinealidadCMaDE/diagnostic_%03d.dat", block_id);
                }
              else if (pba->has_cdm == _TRUE_) {
              sprintf(filename, "LinealidadLCDM/diagnostic_%03d.dat", block_id);
              }

            pw.f_diag = fopen(filename, "w");
            if (pw.f_diag == NULL) {
            class_stop(ppt->error_message, "No se pudo abrir archivo de diagnóstico por bloque. Crear carpeta LinealidadCMaDE o LinealidadLCDM");
            }

            fprintf(pw.f_diag, "k ,  a,  delta_dm,  deltab,   phi \n"  );
            }
          }
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        ppt->has_source_delta_b = _TRUE_;
              ...
              //Claudio Salas Pérez: 06/10/2025        
        if(pba->has_CMaDE == _TRUE_){
          ppt->has_source_CMaDE_delta_dm = _TRUE_;
        }
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if((pba->has_CMaDE == _TRUE_) && (ppt->gauge != synchronous)){
          ppt->has_source_CMaDE_theta_dm = _TRUE_;
        }
      ...
      //CLaudio Salas Pérez: 06/10/2025
      //Reserve indices for perturbed recombination sources
      class_define_index(ppt->index_tp_CMaDE_delta_dm, ppt->has_source_CMaDE_delta_dm, index_type,1);
      class_define_index(ppt->index_tp_CMaDE_theta_dm, ppt->has_source_CMaDE_theta_dm, index_type,1);
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    //Claudio Salas Pérez: 06/10/2025
    /* Materia oscura para CMaDE*/
    class_define_index(ppv->index_pt_CMaDE_delta_dm,pba->has_CMaDE,index_pt,1); /* CMaDE density */
    class_define_index(ppv->index_pt_CMaDE_theta_dm,pba->has_CMaDE && (ppt->gauge == newtonian),index_pt,1); /* cdm velocity */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      //CLaudio Salas Pérez
      if (pba->has_CMaDE == _TRUE_) {
       ppv->y[ppv->index_pt_CMaDE_delta_dm] = ppw->pv->y[ppw->pv->index_pt_CMaDE_delta_dm];
        if (ppt->gauge == newtonian){
       ppv->y[ppv->index_pt_CMaDE_theta_dm] = ppw->pv->y[ppw->pv->index_pt_CMaDE_theta_dm];
        }
      }
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double delta_ur=0.,theta_ur=0.,shear_ur=0.,l3_ur=0.,eta=0.,delta_cdm=0.,delta_CMaDE_dm=0.,alpha, alpha_prime;
  double delta_dr=0;
  double q,epsilon,k2;
  int index_q,n_ncdm,idx;
  double rho_r,rho_m,rho_nu,rho_m_over_rho_r, rho_cdm =0., rho_CMaDE_dm=0.;
  double fracnu,fracg,fracb,fraccdm = 0.,fracidm = 0., fracCMaDEdm=0.;
  double om;
  ...
  if(pba->has_CMaDE == _TRUE_){
    rho_m += ppw->pvecback[pba->index_bg_CMaDE_rho_dm];
  }
  ...
  if (_scalars_) {
    ...
    //Claudio Salas Pérez: Fraction of CMaDE in matter
    if(pba->has_CMaDE == _TRUE_){
      fracCMaDEdm = ppw->pvecback[pba->index_bg_CMaDE_rho_dm]/rho_m;
    }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      //Claudio Salas Pérez: 06/10/2025
      //Condiciones iniciales para CMaDE, para la materia oscura como prueba las dejaré como Cdm
      if(pba->has_CMaDE == _TRUE_){
        // CMaDE density: igual que CDM
             ppw->pv->y[ppw->pv->index_pt_CMaDE_delta_dm] = 3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* CMaDE density */
        //  ppw->pv->y[ppw->pv->index_pt_CMaDE_theta_dm] = 0.0; // velocidad nula en gauge sincrónico
      }
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      //Claudio Salas Pérez: 06/10/2025
      if(pba->has_CMaDE == _TRUE_){
        delta_CMaDE_dm += ppw->pvecback[pba->index_bg_CMaDE_rho_dm] * ppw->pv->y[ppw->pv->index_pt_CMaDE_delta_dm];
        rho_CMaDE_dm += ppw->pvecback[pba->index_bg_CMaDE_rho_dm];
      }
      ...
      if(pba->has_CMaDE == _TRUE_){
        if (rho_CMaDE_dm > 0 ) {
          delta_CMaDE_dm /= rho_CMaDE_dm;
          fracCMaDEdm = rho_CMaDE_dm/rho_m;
        }
      }
    ...
    if(pba->has_CMaDE == _TRUE_){ 
    delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur+rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fracCMaDEdm*delta_CMaDE_dm))/(1.+rho_m_over_rho_r);
    ...
    velocity_tot = ((4./3.)*(fracg*ppw->pv->y[ppw->pv->index_pt_theta_g]+fracnu*theta_ur) + rho_m_over_rho_r*fracb*ppw->pv->y[ppw->pv->index_pt_theta_b])/(1.+rho_m_over_rho_r);
      ...
      //Claudio Salas Pérez: 06/10/2025
      if (pba->has_CMaDE == _TRUE_) {
          ppw->pv->y[ppw->pv->index_pt_CMaDE_delta_dm] -= 3.*a_prime_over_a*alpha;
          ppw->pv->y[ppw->pv->index_pt_CMaDE_theta_dm] = k*k*alpha;
    ...
    if (ppt->gauge == newtonian) {
      ...
      /* in principle we could get phi from the constrain equation:
         ...
         second equation below (credits to Guido Walter Pettinari). */
            ...
            // Aqui no es necesario cambiar nada en las ecuaciones para Phi y Psi. 
    ...
    if (ppt->has_matter_source_in_current_gauge == _FALSE_) {
      if (ppt->has_source_delta_m == _TRUE_) {
        ...
        if(pba->has_CMaDE == _TRUE_){
           ...
           double M_prime = ppw->pvecback[pba->index_bg_CMaDE_M_prime];
           double rho_dm = ppw->pvecback[pba->index_bg_CMaDE_rho_dm];
           double rho_b = ppw->pvecback[pba->index_bg_rho_b];
           double prefactor = M_prime / (rho_dm + rho_b);

          //Gaguges invariant
          ppw->delta_m += 3. *ppw->pvecback[pba->index_bg_a]*ppw->pvecback[pba->index_bg_H] * ppw->theta_m/k2 + pba->k_c*prefactor*ppw->theta_m/k2;
        }else{
...
int perturbations_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    //Claudio Salas Pérez:07/10/2025
    /* CMaDE contribution */
if (pba->has_CMaDE == _TRUE_) {

  /* Perturbed energy density */
  ppw->delta_rho += ppw->pvecback[pba->index_bg_CMaDE_rho_dm] * y[ppw->pv->index_pt_CMaDE_delta_dm]; // contribution to total perturbed stress-energy

  if (ppt->gauge == newtonian){
  ppw->rho_plus_p_theta = ppw->rho_plus_p_theta + (ppw->pvecback[pba->index_bg_CMaDE_rho_dm])*y[ppw->pv->index_pt_CMaDE_theta_dm]; 
  }
    ppw->rho_plus_p_tot += (ppw->pvecback[pba->index_bg_CMaDE_rho_dm]); //aqui deberiamos agregar la presion de la materia oscura

    if (ppt->has_source_delta_m == _TRUE_) {
        delta_rho_m += ppw->pvecback[pba->index_bg_CMaDE_rho_dm]*y[ppw->pv->index_pt_CMaDE_delta_dm]; // contribution to delta rho_matter
        rho_m += ppw->pvecback[pba->index_bg_CMaDE_rho_dm];
      }
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        if (ppt->gauge == newtonian)
          rho_plus_p_theta_m += ppw->pvecback[pba->index_bg_CMaDE_rho_dm]*y[ppw->pv->index_pt_CMaDE_theta_dm]; // contribution to [(rho+p)theta]_matter
        rho_plus_p_m += (ppw->pvecback[pba->index_bg_CMaDE_rho_dm]);
      }
}
    ...
    */
    ...
    //Aqui se calcula delta_m y theta_m totales que se usan para las funciones fuente 
    // Ya esta incluido CMaDE. Claudio Salas Pérez:08/11/2025
  ...
  /* delta_tot */
    ...
    if (ppt->has_source_delta_cdm == _TRUE_) {
      _set_source_(ppt->index_tp_delta_cdm) = y[ppw->pv->index_pt_delta_cdm]
        ...
        if (ppt->has_source_CMaDE_delta_dm == _TRUE_) {
      _set_source_(ppt->index_tp_CMaDE_delta_dm) = y[ppw->pv->index_pt_CMaDE_delta_dm]
        + 3.*a_prime_over_a*theta_over_k2; // N-body gauge correction
    }
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppw->f_diag != NULL) {
      fprintf(ppw->f_diag, "%.6e %.6e %.6e %.6e\n", k, a, fabs(y[pv->index_pt_delta_cdm]), fabs(y[pv->index_pt_delta_b]));
          }
    }
    ...
    //CLaudio Salas Pérez: 06/10/2025
    //Perturbaciones de materia oscura con acoplamiento a energía oscura dinámica (CMaDE)
    //regresar
    if (pba->has_CMaDE == _TRUE_) {

    double M_prime = pvecback[pba->index_bg_CMaDE_M_prime];
    double rho_dm = pvecback[pba->index_bg_CMaDE_rho_dm];
    double prefactor = M_prime / rho_dm;
    
    if(ppt->gauge == newtonian) {
      dy[pv->index_pt_CMaDE_delta_dm] =
        -(y[pv->index_pt_CMaDE_theta_dm] + metric_continuity)
        + pba->k_c* prefactor * y[pv->index_pt_CMaDE_delta_dm];

      dy[pv->index_pt_CMaDE_theta_dm] =
        -a_prime_over_a * y[pv->index_pt_CMaDE_theta_dm]
        + metric_euler
        + pba->k_c * prefactor * y[pv->index_pt_CMaDE_theta_dm];

          if (ppw->f_diag != NULL) {
            fprintf(ppw->f_diag, "%.6e %.6e %.6e %.6e \n", k, a, fabs(y[pv->index_pt_CMaDE_delta_dm]), fabs(y[pv->index_pt_delta_b]));
        }
    }
        
      /** - ----> synchronous gauge: cdm density only (velocity set to zero by definition of the gauge) */
        if (ppt->gauge == synchronous) {
        dy[pv->index_pt_CMaDE_delta_dm] = -metric_continuity + pba->k_c * prefactor * y[pv->index_pt_CMaDE_delta_dm]; /* cdm density */
